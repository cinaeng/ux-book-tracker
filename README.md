...
GitHub Actions가 매일 **KST 11:00**에 순위를 수집하고, GitHub Pages가 대시보드를 서빙합니다. 내 컴퓨터가 꺼져 있어도 알아서 돕니다.
...
.github/workflows/collect.yml 매일 11시 실행 + Pages 배포
```
...
- **수집 시각을 11시로 잡은 이유** — 예스24는 갱신 시각을 공개하지 않습니다. 관측상 오전 9시 시점에 이미 전일(D-1)까지 반영돼 있었으므로, 갱신은 자정~9시 사이에 일어납니다. 09시에 수집하면 경계에 걸려 하루 지난 순위를 기록할 수 있어 11시로 미뤘습니다. 순위가 하루 1회만 바뀌므로 늦게 수집해도 정보 손실은 없습니다.
